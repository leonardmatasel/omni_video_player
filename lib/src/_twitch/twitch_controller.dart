...
class TwitchController extends OmniPlaybackController {
  @override
  ...
  static TwitchController create({
  ...
  @override
  VideoSourceType get videoSourceType => VideoSourceType.twitch;
  ...
  set isSeeking(bool value) {
    _isSeeking = value;
    if (!value && !isLive) callbacks.onSeekEnd?.call(currentPosition);
    notifyListeners();
  }
  ...
  @override
  bool get isFinished => !isLive && duration == currentPosition;
  ...
  Future<void> replay({bool useGlobalController = true}) async {
    await Future.wait([
      ...
      if (!isLive) seekTo(Duration.zero),
  ...
  set volume(double value) {
    _evaluate("player.setVolume($value);");
    _volume = value;
    notifyListeners();
  }
  ...
  void mute() {
    ...
    _evaluate("player.setMuted(true);");
  }
  ...
  void unMute() {
    ...
    _evaluate("player.setMuted(false);");
  }
  ...
  Future<void> seekTo(
    ...
    if (isLive) return; // cannot seek live
    if (position <= duration) {
      ...
      await _evaluate("player.seek(${position.inSeconds});");
  ...
  void startPositionTimer() {
    if (isLive) return; // no timer for live
  ...
  set playbackSpeed(double speed) {
    if (speed <= 0) throw ArgumentError('Playback speed must be > 0');
